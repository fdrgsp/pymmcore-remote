...
import numpy as np
import tifffile
from useq import MDAEvent, MDASequence, PropertyTuple
...
NUCLEI_PATH = Path("/Users/fdrgsp/Desktop/nuc")
...
class MDAEngine(PMDAEngine):
    ...
    def exec_event(self, event: MDAEvent) -> Any:
        ...
        images = list(NUCLEI_PATH.glob('*.tif'))

        finding_idx = True
        while finding_idx:
            idx = np.random.randint(0, len(images))
            if idx not in self._image_indexes:
                finding_idx = False
                self._image_indexes.append(idx)
        
        image = tifffile.imread(images[idx])[:512, :512]

        return EventPayload(image=image)
